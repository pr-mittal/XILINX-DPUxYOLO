...
def generate_images(dset_dir, num_images, dest_dir):
  ...
  for i in tqdm(range(num_images)):
    img, targets, paths, shapes = dataiter.next()
    img = img.numpy().squeeze()
    img_file=os.path.join(dest_dir,'_'+str(i)+'.png')
...
def make_target(build_dir,target,num_images,app_dir):
    ...
    # copy compiled model
    model_path = comp_dir + '/yolov3_' + target + '.xmodel'
